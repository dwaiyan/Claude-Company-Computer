...
const mockAssessments: AssessmentItem[] = [
  { id: 'as-1', title: 'ASP.NET Core 基礎評価', type: '選択', nodeId: 'node-1', timeLimit: 30, passScore: 70, questions: [], createdAt: '2026-06-01' },
  { id: 'as-2', title: 'マイクロサービス設計評価', type: '設計', nodeId: 'node-3', timeLimit: 60, passScore: 75, questions: [], createdAt: '2026-06-03' },
];
const mockContribs: ContribItem[] = [
  { id: 'c-1', userId: 'u-002', type: 'リソース', target: 'Docker Compose 実践デプロイガイド', status: 'pending', createdAt: '2026-06-09' },
  { id: 'c-2', userId: 'u-003', type: 'ノート', target: 'Polly 弹性戦略の深層解析', status: 'pending', createdAt: '2026-06-08' },
];
...
<template>
  <div class="dashboard">
    <header class="app-header"><div class="header-left"><h1>管理画面</h1><nav class="nav-links"><router-link to="/">← 戻る</router-link></nav></div><div class="header-right"><ThemeToggle /></div></header>
    <div class="page-container">
      <div class="page-header"><h1>管理画面</h1></div>

      <div class="stats-row stagger-1">
        <div class="stat-card"><div class="stat-num">{{ stats.total }}</div><div class="stat-label">評価総数</div></div>
        <div class="stat-card"><div class="stat-num">{{ stats.totalRecords }}</div><div class="stat-label">提出記録</div></div>
        <div class="stat-card"><div class="stat-num">{{ stats.avgScore }}</div><div class="stat-label">平均点</div></div>
      </div>
      ...
      <div class="card stagger-2">
        ...
        <div v-for="c in contribs" :key="c.id" style="display:flex;justify-content:space-between;align-items:center;padding:14px 0;border-bottom:1px solid var(--border-subtle);font-size:13px;">
          ...
          <div style="display:flex;gap:8px;">
            <button @click="reviewContrib(c.id, 'approved')" style="padding:4px 14px;border:1px solid var(--color-accent);border-radius:var(--radius-sm);background:transparent;color:var(--color-accent);cursor:pointer;font-family:var(--font-body);font-size:11px;font-weight:400;">承認</button>
            <button @click="reviewContrib(c.id, 'rejected')" style="padding:4px 14px;border:1px solid var(--color-danger);border-radius:var(--radius-sm);background:transparent;color:var(--color-danger);cursor:pointer;font-family:var(--font-body);font-size:11px;font-weight:400;">却下</button>
          </div>
        </div>
        <p v-if="!contribs.length" class="empty">審査待ちはありません</p>
      </div>
    </div>
  </div>
</template>
